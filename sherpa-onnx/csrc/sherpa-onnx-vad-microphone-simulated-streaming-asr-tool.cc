// sherpa-onnx/csrc/sherpa-onnx-vad-microphone-simulated-streaming-asr-tool.cc
...
#include "sherpa-onnx/csrc/sherpa-display-tool.h"
...
sherpa-onnx for real-time speech recognition with stdout-only output.
...
./bin/sherpa-onnx-vad-microphone-simulated-streaming-asr-tool \
...
A model with RTF < 0.2 should work with this program.
  ...
  sherpa_onnx::SherpaDisplayTool display;
  ...
  while (!stop) {
    ...
    while (!vad->Empty()) {
      ...
      display.FinalizeCurrentSentence(current_text);
